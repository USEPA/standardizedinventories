...
def import_table(filepath, skip_lines=0, drop_sheets=[]):
    if filepath[-3:].lower() == 'csv':
        ...
        skip_rows = ''
        if skip_lines > 0: skip_rows = ', skiprows='+str(skip_lines)
        exec('table_dict = {sheet: table_dict.parse(sheet' + skip_rows + ') for sheet in table_dict.sheet_names}')
        if drop_sheets:
            for s in drop_sheets: table_dict.pop(s)
...
for table in required_tables:
    ...
    if os.path.exists(filepath):
        try:
            exec(table + '=' + read_type + '("' + filepath + '")')
        except UnicodeDecodeError:
            exec(table + '=' + read_type + '("' + filepath + '",encoding="latin1")')
        write_type = ''
