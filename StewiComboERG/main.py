...
def main():
    ...
    # force cast skeptical columns based on dtype in config ( FORCE_COLUMN_TYPES )
    for col_name, dtype in FORCE_COLUMN_TYPES.items():
        df[col_name] = df[col_name].astype(dtype)

    # 2
    # if you wish to also keep row that doesn't have any duplicates,
    # the finding duplicates step becomes irrelevant
    ...
    print("Some specific changes based on SRS_ID")
    #Find records where  SRS_ID = 77683 and SRC_ID = 77681  and the same FRS_ID  . The record with SRS_ID 77681 is not changed.
    #In the record with SRC_ID 77683  change FlowAmont by substarcting amont in record 77681 (30-20 =10)
    #Add column CombinedFlowLookup to output  and concatinate (see formula)
    ...
    # reorder columns to similar state of original input columns
    df = order(df, columns)
